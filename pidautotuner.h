...
typedef struct
{
  ...
  bool output;
  float outputValue;
  float seconds, t1, t2, tHigh, tLow;
  float max, min;
...
extern void vPIDAutotunerReset(PIDAutotuner_t *pxPID);
...
extern void vPIDAutotunerStartTuningLoop(PIDAutotuner_t *pxPID, float tm);

// Automatically tune PID
// This function must be run in a loop at the same speed as the PID loop being tuned
// See README for more details - https://github.com/jackw01/arduino-pid-autotuner/blob/master/README.md
extern float fPIDAutotunerTunePID(PIDAutotuner_t *pxPID, float input, float tm);
...
extern float fPIDAutotunerUpdate(PIDAutotuner_t *pxPID,
